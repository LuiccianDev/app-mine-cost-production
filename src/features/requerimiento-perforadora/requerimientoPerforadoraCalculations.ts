import {
  ...
  type RequerimientoPerforadoraResultados,
...
type RequerimientoPerforadoraCalculationData = RequerimientoPerforadoraData &
  Pick<
    SharedData,
    | 'produccionMina'
    | 'alturaBanco'
...
export function calcularRequerimientoPerforadora(
  data: RequerimientoPerforadoraCalculationData,
): RequerimientoPerforadoraResultados {
  const {
    produccionMina,
    tonelajePerforado,
    longuitudTaladro,
  ...
  const numeroPerforadoras =
    (produccionMina / tonelajePerforado) *
    (longuitudTaladro / rendimientoBroca) *
    (tiempoPerforacion / 24) *
    (1 / eficienciaPerforadora)
